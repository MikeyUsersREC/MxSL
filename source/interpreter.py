...
from source.token_types import TT_PLUS, TT_MUL, TT_DIV, TT_MINUS, TT_POW, TT_GTE, TT_LTE, TT_GT, TT_LT, TT_NE, TT_EE, \
    TT_KEYWORD
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        right = res.register(self.visit(node.right_mode, context))
        if res.should_return(): return res
    ...
    def visit_IfNode(self, node, context):
        ...
        for condition, expr, should_return_null in node.cases:
            conditional_value = res.register(self.visit(condition, context))
            if res.should_return():
                return res

            if conditional_value.is_true():
            ...
            if node.else_case:
                else_value, should_return_null = node.else_case
                if res.should_return():
                    return res
    ...
    def visit_FuncDefNode(self, node, context):
        ...
        func_name = node.var_name_tok.value if node.var_name_tok and getattr(node.var_name_tok, 'value') else None
